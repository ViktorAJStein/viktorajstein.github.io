---
title: "Publications"
permalink: /publications/
layout: single
author_profile: true
---

<div id="publist">
  <script src="https://bibbase.org/show?bib=https://raw.githubusercontent.com/ViktorAJStein/viktorajstein.github.io/main/files/Publications.bib&jsonp=1&group0=year&sort=-year&hidemenu=true&fullnames=1"></script>
</div>

<script>
(function () {
  function normalizeLabel(text) {
    return text.replace(/\s+/g, "").toLowerCase();
  }

  function classifyBibBaseLinks() {
    const links = document.querySelectorAll(".bibbase_paper_content a");
    links.forEach((a) => {
      const label = normalizeLabel(a.textContent || "");
      const href = (a.getAttribute("href") || "").toLowerCase();

      a.classList.add("pub-badge");

      if (label.includes("doi") || href.includes("doi.org")) {
        a.classList.add("pub-badge--doi");
      } else if (label.includes("preprint") || href.includes("arxiv.org")) {
        a.classList.add("pub-badge--preprint");
      } else if (label.includes("code") || href.includes("github.com")) {
        a.classList.add("pub-badge--code");
      } else if (label.includes("biblatex") || label.includes("bibtex") || href.endsWith(".bib")) {
        a.classList.add("pub-badge--bib");
      } else if (label.includes("slides")) {
        a.classList.add("pub-badge--slides");
      } else if (label.includes("video")) {
        a.classList.add("pub-badge--video");
      } else if (label.includes("pdf")) {
        a.classList.add("pub-badge--pdf");
      } else if (label.includes("paper")) {
        a.classList.add("pub-badge--paper");
      }
    });
  }

  const observer = new MutationObserver(classifyBibBaseLinks);
  observer.observe(document.body, { childList: true, subtree: true });

  window.addEventListener("load", classifyBibBaseLinks);
  setTimeout(classifyBibBaseLinks, 1000);
  setTimeout(classifyBibBaseLinks, 2500);
})();
</script>

- [J. Bresch](https://www.user.tu-berlin.de/jonasbresch/), **V. Stein**: **Interpolating between Optimal Transport and KL regularized Optimal Transport using Rényi Divergences**. *Results in Mathematics* 81, 23 (2026). [arXiv](https://arxiv.org/abs/2404.18834) · [Journal](https://link.springer.com/article/10.1007/s00025-025-02579-z) · [Code](https://github.com/JJEWBresch/renyi_regularized_OT) · [Slides](https://speakerdeck.com/viktorstein/interpolating-between-optimal-transport-and-kl-regularized-optimal-transport-using-renyi-divergences)

- [R. Duong](https://www.researchgate.net/profile/Richard-Duong), **V. Stein**, [R. Beinert](https://scholar.google.com/citations?user=D-RIm78AAAAJ&hl=en&oi=ao), [J. Hertrich](https://johertrich.github.io/), [G. Steidl](https://page.math.tu-berlin.de/~steidl/): **Wasserstein Gradient Flows of MMD Functionals with Distance Kernel and Cauchy Problems on Quantile Functions**. *ESAIM: Control, Optimisation and Calculus of Variations*. [Abstract](https://arxiv.org/abs/2408.07498) · [PDF](https://doi.org/10.1051/cocv/2025097) · [Code](https://github.com/viktorajstein/MMD_Wasserstein_gradient_flow_on_the_line/tree/main)

- **V. Stein**, [S. Neumayer](https://www.tu-chemnitz.de/mathematik/invimg/), [N. Rux](https://scholar.google.com/citations?user=G8T1CF8AAAAJ&), [G. Steidl](https://page.math.tu-berlin.de/~steidl/): **Wasserstein Gradient Flows for Moreau Envelopes of f-Divergences in Reproducing Kernel Hilbert Spaces**. *Analysis and Applications* 24(1), 21–65 (2026). [Journal version](https://www.worldscientific.com/doi/10.1142/S0219530525500162) · [arXiv](https://arxiv.org/abs/2402.04613) · [Code](https://github.com/viktorajstein/Regularized_f_Divergence_Particle_Flows/tree/main)

- [R. Duong](https://www.researchgate.net/profile/Richard-Duong), [N. Rux](https://scholar.google.com/citations?user=G8T1CF8AAAAJ), **V. Stein**, [G. Steidl](https://page.math.tu-berlin.de/~steidl/): **Wasserstein Gradient Flows of MMD Functionals with Distance Kernels under Sobolev Regularization**. *Philosophical Transactions of the Royal Society A*, special issue “Partial Differential Equations in Data Science”. [Abstract](https://arxiv.org/abs/2411.09848) · [PDF](https://arxiv.org/pdf/2411.09848) · [Journal version](https://doi.org/10.1098/rsta.2024.0243)

- **V. Stein**, [Wuchen Li](https://people.math.sc.edu/wuchen/): **Accelerated Stein Variational Gradient Flow**. In: *Proceedings of GSI’25: Geometric Science of Information* (LNCS 16034). [arXiv](https://arxiv.org/abs/2503.23462) · [Code](https://github.com/viktorajstein/Accelerated_Stein_Variational_Gradient_Flows) · [Slides](https://speakerdeck.com/viktorajstein/accelerated-stein-variational-gradient-flow) · [Video](https://youtu.be/_N_JfSOYNps) · [Published version](https://link.springer.com/book/10.1007/978-3-032-03921-7)

## Preprints

- **SympFormer: Accelerated attention blocks via Inertial Dynamics on Density Manifolds**  
  joint work with [Wuchen Li](https://people.math.sc.edu/wuchen/) and [Gabriele Steidl](https://page.math.tu-berlin.de/~steidl/)  
  [PDF](https://arxiv.org/abs/2603.16535) · [Code](https://github.com/ViktorAJStein/SympFormer)

- **Well-Posed KL-Regularized Control via Wasserstein and Kalman-Wasserstein KL Divergences**  
  joint work with [Adwait Datar](https://www.tuhh.de/dsf/team/adwait-datar) and [Nihat Ay](https://www.tuhh.de/dsf/team/prof-dr-nihat-ay), submitted to ICML’26  
  [PDF](https://arxiv.org/abs/2602.02250)

- **Approximating f-Divergences with Rank Statistics**  
  joint work with [José Manuel de Frutos](https://josemanuel22.github.io), submitted to ICML’26  
  [PDF](https://arxiv.org/abs/2601.22784) · [Code](https://anonymous.4open.science/r/rsfdiv-2C8A/)

- **Towards understanding Accelerated Stein Variational Gradient Flow – Analysis of Generalized Bilinear Kernels for Gaussian target distributions**  
  joint work with [Wuchen Li](https://people.math.sc.edu/wuchen/), submitted to JMLR  
  [PDF](https://arxiv.org/pdf/2509.04008) · [Code](https://github.com/viktorajstein/Accelerated_Stein_Variational_Gradient_Flows)


<script src="https://bibbase.org/show?bib=https://raw.githubusercontent.com/ViktorAJStein/viktorajstein.github.io/main/files/Publications.bib&jsonp=1&group0=year&sort=-year&hidemenu=true&theme=simple"></script>
